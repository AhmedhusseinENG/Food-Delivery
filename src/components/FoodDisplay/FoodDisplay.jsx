...
const FoodDisplay = ({category}) => {
  ...
  return (
        ...
        <input  onChange={(e)=> handelSearch(e)} type="text" placeholder='Search your favorite dish'/>
        </div>
            <div className="food-display-list">
                {food_list.filter((item,index)=> {
                 return   search.toLowerCase() === '' ? item : item.name.toLowerCase().includes(search);
                    ...
                    
                    {
                     ...
                     if(category === "All" || category === item.category){
                         return <FoodItem key={index} {...item}  name={highlightMatch(item.name, search)} /> 
                     }
